...
from model import common_pams
from utils.quant_ops import conv3x3
from utils.quant_ops import pams_quant_act
from utils.quant_ops import quant_conv3x3

def make_model(args, parent=False):
    return PAMS_EDSR(args)
...
class PAMS_ResBlock(nn.Module):
    def __init__(self, conv, n_feats, kernel_size, bias=False, 
        ...
        self.shortcut = common_pams.ShortCut()
...
class PAMS_EDSR(nn.Module):
    def __init__(self, args, conv=quant_conv3x3, bias = False, k_bits = 32):
        ...
        self.sub_mean = common_pams.MeanShift(args.rgb_range)
        self.add_mean = common_pams.MeanShift(args.rgb_range, sign=1)
        ...
        # define tail module
        m_tail = [
            common_pams.Upsampler(conv3x3, scale, n_feats, act=False),
            nn.Conv2d(
